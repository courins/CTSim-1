function [x, phis, rmsds] = reconPMBPwlsLALMOs14( y, w, geom, beta, pfun, itnlim, delta, numos, img0, burner  )
...
for itn = 1 : itnlim
    ...
    for isub = 1 : numos
        ...
        for j = 1:5
            ...
            dR = S(x);
            if nargin >= 10
                dR( burner ) = dR( burner ) / 10;
            end
            
            x = x - ( beta * dR + rho * precom .* ( x - xold ) + s  ) ./ ( beta * T(x) + rho * precom );
            x( x < 0) = 0;
            x( isnan(x) ) = 0;
            x( isinf(x) ) = 0;
        end
